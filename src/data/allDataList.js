let allDataList = [
  {
    src: require('../img/호두마늘볶음.jpg'),
    id: 1,
    hash: ['#밑반찬', '#매실청', '#단짠단짠', '#안주'],
    name: '호두마늘볶음',
    view: 312,
    ctg: ['반찬', '채소', '볶음', '채식', '후라이팬'],
    date: '2021-01-01',
    recipe: ['후라이팬에', '마늘과 호두를', '볶습니다'],
  },
  {
    src: require('../img/햄마요덮밥.jpg'),
    id: 2,
    hash: ['#덮밥요리', '#오뚜기햄', '#골드마요네스'],
    name: '햄마요덮밥',
    view: 310,
    ctg: ['밥', '재료기타', '방법기타', '스피드', '키즈푸드', '간단 꿀조합', '후라이팬'],
    date: '2021-01-02',
    recipe: ['햄을 볶고', '밥에 올리고', '마요네즈를 짜줍니다'],
  },
  {
    src: require('../img/푸팟퐁커리.jpg'),
    id: 3,
    hash: ['#태국식', '#게맛살', '#오뚜기카레'],
    name: '푸팟퐁커리',
    date: '2021-01-03',
    view: 320,
    ctg: ['밥', '계란', '방법기타', '부드러운 식감', '간단 꿀조합', '카레데이', '냄비'],
    recipe: ['맛살을 찢고', '밥위에 올리고', '카레를 부어줍니다'],
  },
  {
    src: require('../img/토마토 냉파스타.jpg'),
    id: 4,
    hash: ['#파스타샐러드', '#키즈셰프', '#토마토퓨레'],
    name: '토마토 냉파스타',
    date: '2021-01-04',
    view: 313,
    ctg: ['면', '샐러드', '재료기타', '방법기타', '간단 꿀조합', '키즈셰프', '냄비', '키즈푸드'],
    recipe: ['면을 삶고', '찬물에 행구고', '토마토 소스를 부어줍니다'],
  },
  {
    src: require('../img/북어채 무침.jpg'),
    id: 5,
    hash: ['#밑반찬', '#매실청', '#집밥'],
    name: '북어채 무침',
    date: '2021-01-05',
    view: 312,
    ctg: ['반찬', '해물', '무침·비빔', '다이어트', '도구기타', '명절'],
    recipe: ['매실청과', '북어채를', '무칩니다'],
  },
  {
    src: require('../img/토마토 미트볼파스타.jpg'),
    id: 6,
    hash: ['#미트볼X파스타면', '#오즈키친', '#스파게티'],
    name: '토마토 미트볼파스타',
    date: '2021-01-06',
    view: 500,
    ctg: ['면', '재료기타', '방법기타', '1인분', '간단 꿀조합', '요린이를 위하여', '크리스마스', '냄비'],
    recipe: ['미트볼을 삶고', '면을 삶고', '삶았던 재료 위에 토마토 소스를 부어줍니다'],
  },
  {
    src: require('../img/콘치즈.jpg'),
    id: 7,
    hash: ['#치즈듬뿍', '#단짠단짠', '#골드마요네스', '#안주'],
    name: '콘치즈',
    date: '2021-01-07',
    view: 452,
    ctg: ['종류기타', '유제품', '방법기타', '스피드', '야식', '간식', '파티', '키즈셰프', '키즈푸드', '간단 꿀조합', '요린이를 위하여', '크리스마스', '후라이팬'],
    recipe: ['후라이팬에 옥수수통조림을 까서 넣고', ' 치즈도 같이 올려주고', ' 약불에서 볶아줍니다'],
  },
  {
    src: require('../img/카레 샥슈카.jpg'),
    id: 8,
    hash: ['#3일숙성카레', '#에그인헬', '#계란요리'],
    name: '카레 샥슈카',
    date: '2021-01-08',
    view: 380,
    ctg: ['밥', '계란', '국탕찌개', '셰프의 팁! 간편식 업그레이드', '캠핑', '카레데이', '냄비'],
    recipe: ['카레를 만들고', ' 밥 위에', ' 계란 후라이를 올려줍니다'],
  },
  {
    src: require('../img/카레 비프 스튜.jpg'),
    id: 9,
    hash: ['#3일숙성카레', '#카레맛집', '#쇠고기'],
    name: '카레 비프 스튜',
    date: '2021-01-09',
    view: 345,
    ctg: ['밥', '쇠고기', '국탕찌개', '셰프의 팁! 간편식 업그레이드', '캠핑', '카레데이', '냄비'],
    recipe: ['카레를 만들고', ' 소고기도 넣어주고', ' 밥 위에 올려줍니다'],
  },
  {
    src: require('../img/치킨 카레 타코.jpg'),
    id: 10,
    hash: ['#맥시코', '#밀또띠아', '#닭가슴살'],
    name: '치킨 카레 타코',
    date: '2021-01-10',
    view: 412,
    ctg: ['종류기타', '닭고기', '국탕찌개', '파티', '키즈푸드', '카레데이', '냄비', '오븐', '생일'],
    recipe: ['닭을 튀기고', ' 또띠아 위에', ' 채소와 함께 재료를 싸줍니다'],
  },
  {
    src: require('../img/차돌박이 된장찌개.jpg'),
    id: 11,
    hash: ['#셰프의팁', '#즉석국', '#캠핑', '#된장'],
    name: '차돌박이 된장찌개',
    date: '2021-01-11',
    view: 454,
    ctg: ['국물', '쇠고기', '국탕찌개', '해장', '냄비'],
    recipe: ['된장과 채소를 넣고', ' 차돌박이도 같이 넣고 볶아준다음', ' 물을 붓고 끓입니다'],
  },
  {
    src: require('../img/진라면밥.jpg'),
    id: 12,
    hash: ['#셰프의팁', '#라면밥', '#냄비밥', '#분식'],
    name: '진라면밥',
    date: '2021-01-12',
    view: 400,
    ctg: ['국물', '면', '밥', '재료기타', '국탕찌개', '스피드', '야식', '캠핑', '간단 꿀조합', '요린이를 위하여', '해장', '1인분', '냄비'],
    recipe: ['라면을 끓이고', ' 밥도 넣고', ' 김가루도 취향에 맞게 뿌려줍니다'],
  },
  {
    src: require('../img/육개장 칼국수.jpg'),
    id: 13,
    hash: ['#셰프의팁', '#즉석국', '#집밥'],
    name: '육개장 칼국수',
    date: '2021-01-13',
    view: 312,
    ctg: ['국물', '면', '쇠고기', '국탕찌개', '야식', '간단 꿀조합', '해장', '냄비'],
    recipe: ['육개장을 넣고', ' 면을 넣고', ' 삶아줍니다'],
  },
  {
    src: require('../img/유자 연근조림.jpg'),
    id: 14,
    hash: ['#집밥', '#밑반찬', '#꿀유자차'],
    name: '유자 연근조림',
    date: '2021-01-14',
    view: 360,
    ctg: ['반찬', '채소', '조림', '도시락', '채식', '후라이팬'],
    recipe: ['유자청과', '연근을', '조립니다'],
  },
  {
    src: require('../img/옥수수전.jpg'),
    id: 15,
    hash: ['#스위트콘', '#부침개', '#연유'],
    name: '옥수수전',
    date: '2021-01-15',
    view: 320,
    ctg: ['반찬', '재료기타', '방법기타', '간단 꿀조합', '키즈푸드', '후라이팬', '명절'],
    recipe: ['캔옥수수를 까서', '밀가루와 함께', '부쳐줍니다'],
  },
  {
    src: require('../img/오이미역냉국.jpg'),
    id: 16,
    hash: ['#집밥', '#밑반찬', '#옛날미역'],
    name: '오이미역냉국',
    date: '2021-01-16',
    view: 322,
    ctg: ['국물', '채소', '국탕찌개', '다이어트', '채식', '도구기타'],
    recipe: ['오이를 썰고', '미역과 함께 넣고', '식초와 물을 넣습니다'],
  },
  {
    src: require('../img/에그마요 샌드위치.jpg'),
    id: 17,
    hash: ['#갈릭아이올리', '#계란요리', '#브런치'],
    name: '에그마요 샌드위치',
    date: '2021-01-17',
    view: 512,
    ctg: ['종류기타', '계란', '방법기타', '브런치', '간식', '도시락', '1인분', '키즈셰프', '키즈푸드', '부드러운 식감', '간단 꿀조합', '요린이를 위하여', '도구기타'],
    recipe: ['삶은 계란을', '마요네즈에 버무리고', '빵 사이에 넣습니다'],
  },
  {
    src: require('../img/시나몬롤.jpg'),
    id: 18,
    hash: ['#베이킹', '#찹살호떡믹스', '#에어프라이어'],
    name: '시나몬롤',
    date: '2021-01-18',
    view: 498,
    ctg: ['디저트', '재료기타', '방법기타', '브런치', '간식', '크리스마스', '생일', '오븐', '에어프라이어', '파티'],
    recipe: ['찹살호떡믹스와 함께 반죽을 해서', '에어프라이어에', '180도로 데워줍니다'],
  },
  {
    src: require('../img/쇠고기채소말이.jpg'),
    id: 19,
    hash: ['#불고기', '#도시락', '#파티'],
    name: '쇠고기채소말이',
    date: '2021-01-19',
    view: 327,
    ctg: ['반찬', '채소', '쇠고기', '구이', '캠핑', '파티', '도시락', '후라이팬', '크리스마스'],
    recipe: ['쇠고기 불고기를 볶고', '채소와 함께', '말아줍니다'],
  },
  {
    src: require('../img/삼겹살 순두부짜글이.jpg'),
    id: 20,
    hash: ['#얼큰칼칼', '#오늘밥상', '#한식', '#된장'],
    name: '삼겹살 순두부짜글이',
    date: '2021-01-20',
    view: 612,
    ctg: ['국물', '돼지고기', '국탕찌개', '해장', '냄비'],
    recipe: ['된장에 채소와', '삼겹살을 볶고', '물을 넣고 끓여줍니다'],
  },
  {
    src: require('../img/바지락찜.jpg'),
    id: 21,
    hash: ['#안주', '#해물찜', '#얼큰칼칼'],
    name: '바지락찜',
    date: '2021-01-21',
    view: 355,
    ctg: ['종류기타', '해물', '찜', '야식', '해장', '냄비'],
    recipe: ['찜기에 싱싱한 바지락을 넣고', '냄비에 물을 넣고', '쪄줍니다'],
  },
  {
    src: require('../img/문어 와사비 냉채.jpg'),
    id: 22,
    hash: ['#와사비소스', '#샐러드', '#무침'],
    name: '문어 와사비 냉채',
    date: '2021-01-22',
    view: 311,
    ctg: ['샐러드', '채소', '해물', '무침·비빔', '다이어트', '채식', '도구기타'],
    recipe: ['삶은 문어와', '각종 채소와 와사비를', '무칩니다'],
  },
  {
    src: require('../img/몬테크리스토.jpg'),
    id: 23,
    hash: ['#오뚜기쨈', '#토스트', '#치즈듬뿍'],
    name: '몬테크리스토',
    date: '2021-01-23',
    view: 433,
    ctg: ['디저트', '유제품', '방법기타', '브런치', '스피드', '간식', '키즈푸드', '간단 꿀조합', '요린이를 위하여', '크리스마스', '후라이팬', '오븐'],
    recipe: ['빵위에 잼과', '치즈를 넣고', '구워줍니다'],
  },
  {
    src: require('../img/매콤 쉬림프 카레라이스.jpg'),
    id: 24,
    hash: ['#카레맛집', '#새우', '#스리라챠소스'],
    name: '매콤 쉬림프 카레라이스',
    date: '2021-01-24',
    view: 545,
    ctg: ['밥', '해물', '국탕찌개', '캠핑', '1인분', '카레데이', '냄비'],
    recipe: ['카레를 끓이고', '스리라챠소스와 함께 새우를 볶고', '밥 위에 올려줍니다'],
  },
  {
    src: require('../img/떡만둣국.jpg'),
    id: 25,
    hash: ['#XO만두', '#따끈한국물', '#설날'],
    name: '떡만둣국',
    date: '2021-01-25',
    view: 555,
    ctg: ['국물', '재료기타', '국탕찌개', '스피드', '1인분', '간단 꿀조합', '요린이를 위하여', '명절', '냄비'],
    recipe: ['만두와 떡을 넣고', '끓여준다음', '계란지단을 올려줍니다'],
  },
  {
    src: require('../img/돈코츠오라면.jpg'),
    id: 26,
    hash: ['#오라면X부산식돼지국밥', '#야식', '#캠핑', '#분식', '#간단 꿀조합'],
    name: '돈코츠오라면',
    date: '2021-01-26',
    view: 377,
    ctg: ['국물', '돼지고기', '국탕찌개', '스피드', '해장', '셰프의 팁! 간편식 업그레이드', '간단 꿀조합', '냄비', '1인분'],
    recipe: ['돼지국밥 간편식을 넣어주고', '끓인다음 라면을 넣어주고', '밥도 같이 말아먹습니다'],
  },
  {
    src: require('../img/돈까스샌드위치.jpg'),
    id: 27,
    hash: ['#식빵요리', '#도시락', '#마요네스'],
    name: '돈까스샌드위치',
    date: '2021-01-27',
    view: 458,
    ctg: ['종류기타', '돼지고기', '방법기타', '간식', '캠핑', '키즈푸드', '후라이팬'],
    recipe: ['돈까스를 튀기고', '빵 사이에', '끼워줍니다'],
  },
  {
    src: require('../img/더치베이비 팬케이크.jpg'),
    id: 28,
    hash: ['#핫케이크', '#시나몬', '#브런치'],
    name: '더치베이비 팬케이크',
    date: '2021-01-28',
    view: 366,
    ctg: ['종류기타', '재료기타', '방법기타', '브런치', '키즈푸드', '생일', '크리스마스', '오븐'],
    recipe: ['시나몬을 핫케이크 믹스와 반죽해서', '오븐에', '구워줍니다'],
  },
  {
    src: require('../img/단밤 크림스프.jpg'),
    id: 29,
    hash: ['#밤', '#크림스프', '#분말스프'],
    name: '단밤 크림스프',
    date: '2021-01-29',
    view: 350,
    ctg: ['종류기타', '채소', '국탕찌개', '부드러운 식감', '냄비'],
    recipe: ['삶은 밤을', '크림 스프에', '넣어줍니다'],
  },
  {
    src: require('../img/녹차핫케이크와 딸기콩피.jpg'),
    id: 30,
    hash: ['#홈카페', '#브런치', '#계란요리'],
    name: '녹차핫케이크와 딸기콩피',
    date: '2021-01-30',
    view: 390,
    ctg: ['디저트', '계란', '방법기타', '브런치', '간식', '오븐', '부드러운 식감'],
    recipe: ['핫케이크 믹스로 반죽해', '오븐에', '구워줍니다'],
  },
  {
    src: require('../img/골뱅이야채초무침.jpg'),
    id: 31,
    hash: ['#새콤달콤', '#초고추장', '#안주'],
    name: '골뱅이야채초무침',
    date: '2021-02-01',
    view: 300,
    ctg: ['반찬', '해물', '야식', '무침·비빔', '도구기타'],
    recipe: ['골뱅이캔을 따서', '야채와 함께', '초고추장을 넣고 무칩니다'],
  },
  {
    src: require('../img/고추참치계란말이.jpg'),
    id: 32,
    hash: ['#반찬', '#간단한끼', '#계란요리'],
    name: '고추참치계란말이',
    date: '2021-02-02',
    view: 312,
    ctg: ['반찬', '계란', '방법기타', '스피드', '간단 꿀조합', '도시락', '부드러운 식감', '후라이팬'],
    recipe: ['계란을 풀고', '후라이팬에 올려서', '고추참치를 중간에 넣고 말아줍니다'],
  },
  {
    src: require('../img/고추잡채.jpg'),
    id: 33,
    hash: ['#중식', '#굴소스', '#돼지불고기양념'],
    name: '고추잡채',
    date: '2021-02-03',
    view: 475,
    ctg: ['종류기타', '돼지고기', '볶음', '후라이팬'],
    recipe: ['돼지고기에', '굴소스와 불고기 양념을 넣고 볶아줍니다', '꽃빵도 함께 먹습니다'],
  },
  {
    src: require('../img/고사리 갈릭볶음밥.jpg'),
    id: 34,
    hash: ['#마늘의민족', '#알리오올리오', '#한그릇'],
    name: '고사리 갈릭볶음밥',
    date: '2021-02-04',
    view: 333,
    ctg: ['밥', '채소', '방법기타', '채식', '다이어트', '후라이팬'],
    recipe: ['고사리와 마늘을', '후라이팬에 넣고', '밥과 같이 볶아줍니다'],
  },
  {
    src: require('../img/비어치킨.jpg'),
    id: 35,
    hash: ['#비어', '#치킨'],
    name: '비어치킨',
    date: '2021-02-05',
    view: 196,
    ctg: ['종류기타', '닭고기', '튀김', '야식', '파티', '캠핑', '크리스마스', '생일', '오븐', '에어프라이어'],
    recipe: ['치킨 반죽에 맥주를 넣고', '오븐 또는 에어프라이어에', '180도로 구워줍니다'],
  },
  {
    src: require('../img/진라거치아바타.jpg'),
    id: 36,
    hash: ['#치아바타'],
    name: '진라거치아바타',
    date: '2021-02-06',
    view: 211,
    ctg: ['종류기타', '재료기타', '방법기타', '오븐'],
    recipe: ['빵 반죽에', '맥주를 넣고', '오븐에 구워줍니다'],
  },
  {
    src: require('../img/비어브레드.jpg'),
    id: 37,
    hash: ['#브레드'],
    name: '비어브레드',
    date: '2021-02-07',
    view: 165,
    ctg: ['종류기타', '재료기타', '방법기타', '오븐'],
    recipe: ['빵 반죽에', '맥주를 넣고', '오븐에 구워줍니다'],
  },
  {
    src: require('../img/전자레인지브라우니.jpg'),
    id: 38,
    hash: ['#핫케이크믹스', '#핫쵸코', '#브라우니'],
    name: '전자레인지브라우니',
    date: '2021-02-08',
    view: 200,
    ctg: ['디저트', '재료기타', '방법기타', '간식', '파티', '키즈푸드', '부드러운 식감', '생일', '전자레인지', '오븐'],
    recipe: ['브라우니믹스를 반죽해', '랩을 감싸주고', '전자레인지에 돌립니다'],
  },
  {
    src: require('../img/유자고구마샐러드.jpg'),
    id: 39,
    hash: [],
    name: '유자고구마샐러드',
    date: '2021-02-09',
    view: 100,
    ctg: ['샐러드', '채소', '방법기타', '브런치', '다이어트', '채식', '도구기타'],
    recipe: ['삶은 고구마에', '유자청을 올리고', '드레싱을 뿌려줍니다'],
  },
  {
    src: require('../img/팥스타.jpg'),
    id: 40,
    hash: ['#팥칼국수', '#동지', '#겨울간식'],
    name: '팥스타',
    date: '2021-02-10',
    view: 66,
    ctg: ['디저트', '재료기타', '방법기타', '간식', '명절', '냄비'],
    recipe: ['인스턴트 팥죽을 까서', '삶은 칼국수면에', '붓습니다'],
  },
  {
    src: require('../img/오이무침.jpg'),
    id: 41,
    hash: ['#반찬', '#오이무침', '#다시마식초'],
    name: '오이무침',
    date: '2021-02-11',
    view: 75,
    ctg: ['반찬', '채소', '무침·비빔', '도시락', '채식', '다이어트', '명절', '도구기타'],
    recipe: ['오이와', '식초를 넣고', '고춧가루와 함께 무칩니다'],
  },
  {
    src: require('../img/연어장.jpg'),
    id: 42,
    hash: ['#간편장아찌', '#연어', '#간편요리'],
    name: '연어장',
    date: '2021-02-12',
    view: 200,
    ctg: ['반찬', '해물', '방법기타', '부드러운 식감', '도구기타'],
    recipe: ['연어를 넣고', '달달한 간장과 함께', '숙성시킵니다'],
  },
  {
    src: require('../img/케일장아찌.jpg'),
    id: 43,
    hash: ['#장아찌', '#케일장아찌', '#간편장아찌소스'],
    name: '케일장아찌',
    date: '2021-02-13',
    view: 80,
    ctg: ['반찬', '채소', '방법기타', '채식', '다이어트', '도구기타'],
    recipe: ['케일을 넣고', '간장과 함께', '숙성시킵니다'],
  },
  {
    src: require('../img/두부참치마요샌드위치.jpg'),
    id: 44,
    hash: ['#참치', '#샌드위치', '#1/2하프마요네스'],
    name: '두부참치마요샌드위치',
    date: '2021-02-14',
    view: 111,
    ctg: ['종류기타', '재료기타', '방법기타', '셰프의 팁! 간편식 업그레이드', '간단 꿀조합', '요린이를 위하여', '도구기타'],
    recipe: ['참치와 마요네즈를 섞고', '구운 두부와 함께', '빵 사이에 끼워줍니다'],
  },
  {
    src: require('../img/오트밀토마토리조또.jpg'),
    id: 45,
    hash: ['#다이어트', '#닭가슴살', '#브런치'],
    name: '오트밀토마토리조또',
    date: '2021-02-15',
    view: 100,
    ctg: ['종류기타', '닭고기', '방법기타', '브런치', '다이어트', '후라이팬'],
    recipe: ['토마토와', '닭가슴살을', '후라이팬에 조려줍니다'],
  },
  {
    src: require('../img/단호박시나몬그라탕.jpg'),
    id: 46,
    hash: ['#단호박', '#그라탕', '#1/2하프마요네스'],
    name: '단호박시나몬그라탕',
    date: '2021-02-16',
    view: 160,
    ctg: ['종류기타', '채소', '방법기타', '채식', '다이어트', '크리스마스', '오븐'],
    recipe: ['단호박과 시나몬을 넣고', '마요네즈와 함께', '오븐에서 구워줍니다'],
  },
  {
    src: require('../img/순후추떡국라면.jpg'),
    id: 47,
    hash: ['#떡국', '#옛날쌀떡국', '#설날', '#분식', '라면'],
    name: '순후추떡국라면',
    date: '2021-02-17',
    view: 80,
    ctg: ['국물', '면', '계란', '국탕찌개', '스피드', '요린이를 위하여', '명절', '냄비', '분식'],
    recipe: ['라면을 끓이고', '중간에 떡을 넣고', '후추도 뿌려줍니다'],
  },
  {
    src: require('../img/어묵 진짬뽕.jpg'),
    id: 48,
    hash: ['#진짬뽕', '#어묵', '#꿀조합', '#분식', '라면'],
    name: '어묵 진짬뽕',
    date: '2021-02-18',
    view: 122,
    ctg: ['국물', '면', '분식', '해물', '국탕찌개', '야식', '스피드', '캠핑', '해장', '냄비', '요린이를 위하여'],
    recipe: ['진짬뽕을 냄비에 넣고', '중간에 어묵도 같이 넣고', '끓여줍니다'],
  },
  {
    src: require('../img/야끼소바빵.jpg'),
    id: 49,
    hash: ['#철판뽀끼', '#야끼소바', '#일본요리'],
    name: '야끼소바빵',
    date: '2021-02-19',
    view: 120,
    ctg: ['면', '채소', '돼지고기', '볶음', '야식', '1인분', '후라이팬'],
    recipe: ['야끼소바 컵라면을 끓여주고', '후라이팬에', '베이컨과 함께 볶습니다'],
  },
  {
    src: require('../img/돈까스 김치나베 라면.jpg'),
    id: 50,
    hash: ['#열라면', '#돈까스', '#돈까스김치나베'],
    name: '돈까스 김치나베 라면',
    date: '2021-01-19',
    view: 201,
    ctg: ['밥', '반찬', '국물', '면', '분식', '돼지고기', '튀김', '해장', '에어프라이어', '냄비'],
    recipe: ['돈까스를 튀기고', '열라면에 김치를 넣고', '마지막에 돈까스를 올려줍니다'],
  },
  {
    src: require('../img/진짬뽕라면볶음밥.jpg'),
    id: 51,
    hash: ['#진짬뽕', '#볶음밥', '#라면볶음밥'],
    name: '진짬뽕라면볶음밥',
    date: '2021-01-18',
    view: 188,
    ctg: ['밥', '면', '계란', '볶음', '스피드', '야식', '후라이팬'],
    recipe: ['진짬뽕을 스프와 함께 부숴서', '밥과 함께', '후라이팬에 기름을 두르고 볶습니다'],
  },
  {
    src: require('../img/크림 진짬뽕.jpg'),
    id: 52,
    hash: ['#크림진짬뽕', '#진짬뽕', '#라면맛집'],
    name: '크림 진짬뽕',
    date: '2021-01-16',
    view: 133,
    ctg: ['면', '유제품', '해물', '국탕찌개', '분식', '야식', '냄비', '후라이팬'],
    recipe: ['진짬뽕을 냄비에 넣고', '중간에 크림과 함께', '끓여줍니다'],
  },
  {
    src: require('../img/열라떡볶이.jpg'),
    id: 53,
    hash: ['#열라면X떡국떡', '#분식집', '#라면맛집', '#떡볶이'],
    name: '열라떡볶이',
    date: '2021-01-16',
    view: 133,
    ctg: ['면', '재료기타', '국탕찌개', '분식', '야식', '냄비', '볶음', '1인분'],
    recipe: ['떡과 고추장과 물을 넣고', '떡볶이가 완성될쯤에', '라면을 넣고 조립니다'],
  },
  {
    src: require('../img/모듬만두덮밥.jpg'),
    id: 54,
    hash: ['#XO만두', '#왕초보', '#셰프의팁', '#덮밥'],
    name: '모듬만두덮밥',
    date: '2021-01-11',
    view: 134,
    ctg: ['밥', '계란', '방법기타', '간단 꿀조합', '요린이를 위하여', '셰프의 팁! 간편식 업그레이드', '냄비'],
    recipe: ['찐 모듬만두와', '계란과 간장을', '밥 위에 올려줍니다'],
  },
  {
    src: require('../img/만두전골.jpg'),
    id: 55,
    hash: ['#XO만두', '#따끈한국물', '#만능소스', '#전골'],
    name: '만두전골',
    date: '2021-01-16',
    view: 133,
    ctg: ['국물', '해물', '국탕찌개', '해장', '야식', '냄비'],
    recipe: ['만두와 소스를', '전골 냄비에 넣고', '채소와 함께 끓여줍니다'],
  },
  {
    src: require('../img/완자꼬치.jpg'),
    id: 56,
    hash: ['#XO만두', '#굴림만두', '#캠핑', '#꼬치', '#완자'],
    name: '완자꼬치',
    date: '2021-01-02',
    view: 177,
    ctg: ['종류기타', '재료기타', '구이', '캠핑', '파티', '후라이팬', '생일'],
    recipe: ['만두와 완자를', '후라이팬에 구운 다음', '꼬치에 꽂습니다'],
  },
  {
    src: require('../img/깐풍만두.jpg'),
    id: 57,
    hash: ['#XO만두', '#고추맛기름', '#중식', '#깐풍'],
    name: '깐풍만두',
    date: '2021-01-01',
    view: 165,
    ctg: ['종류기타', '재료기타', '구이', '후라이팬'],
    recipe: ['만두를', '깐풍소스와 함께', '후라이팬에 볶습니다'],
  },
  {
    src: require('../img/만두덮밥.jpg'),
    id: 58,
    hash: ['#잡채만두', '#계란요리', '#단짠단짠', '#덮밥'],
    name: '만두덮밥',
    date: '2021-01-01',
    view: 111,
    ctg: ['밥', '계란', '방법기타', '간단 꿀조합', '냄비'],
    recipe: ['찐 만두와', '계란과 간장을', '밥 위에 올려줍니다'],
  },
  {
    src: require('../img/흑임자 크림스프.jpg'),
    id: 59,
    hash: ['#크림스프X검은깨', '#스프맛집', '#브런치', '#간단 꿀조합', '#흑임자'],
    name: '흑임자 크림스프',
    date: '2020-12-24',
    view: 121,
    ctg: ['종류기타', '유제품', '방법기타', '브런치', '간단 꿀조합', '요린이를 위하여', '셰프의 팁! 간편식 업그레이드', '크리스마스', '냄비'],
    recipe: ['흑임자와', '크림스프를', '냄비에 넣고 끓여줍니다'],
  },
  {
    src: require('../img/햄버그 스테이크 샐러드.jpg'),
    id: 60,
    hash: ['#햄버그X채소', '#오즈키친', '#왕초보', '#간단 꿀조합', '#샐러드'],
    name: '햄버그 스테이크 샐러드',
    date: '2020-12-24',
    view: 121,
    ctg: ['샐러드', '쇠고기', '방법기타', '스피드', '요린이를 위하여', '간단 꿀조합', '키즈셰프', '키즈푸드', '도구기타'],
    recipe: ['대운 햄버그 스테이크를', '채소와 함께', '드레싱을 뿌려줍니다'],
  },
  {
    src: require('../img/한라산볶음밥.jpg'),
    id: 61,
    hash: ['#볶음밥X라조장', '#계란요리', '#치즈듬뿍', '#간단 꿀조합'],
    name: '한라산볶음밥',
    date: '2020-12-25',
    view: 133,
    ctg: ['밥', '계란', '볶음', '키즈푸드', '간단 꿀조합', '후라이팬'],
    recipe: ['라조장과 계란을', '치즈도 함께 넣어서', '후라이팬에 볶습니다'],
  },
  {
    src: require('../img/토마토 미트볼그라탕.jpg'),
    id: 62,
    hash: ['#미트볼X치즈', '#그라탕', '#치즈듬뿍', '#간단 꿀조합'],
    name: '토마토 미트볼그라탕',
    date: '2020-12-01',
    view: 161,
    ctg: ['종류기타', '돼지고기', '방법기타', '키즈푸드', '1인분', '오븐'],
    recipe: ['구운 미트볼 위에', '토마토 소스와 치즈를 넣고', '오븐에 구워줍니다'],
  },
  {
    src: require('../img/초콜릿피자.jpg'),
    id: 63,
    hash: ['#냉동피자X초콜릿', '#바나나', '#단짠단짠', '#간단 꿀조합'],
    name: '초콜릿피자',
    date: '2020-12-25',
    view: 133,
    ctg: ['디저트', '재료기타', '방법기타', '키즈푸드', '간단 꿀조합', '간식', '셰프의 팁! 간편식 업그레이드', '파티', '생일', '크리스마스', '전자레인지', '오븐', '에어프라이어'],
    recipe: ['냉동피자를 돌려서', '바나나와 초콜릿을 올리고', '다시 전자레인지나 에어프라이어에 조리합니다'],
  },
  {
    src: require('../img/진저라떼.jpg'),
    id: 64,
    hash: ['#꿀생강차X우유', '#홈카페', '#생강청', '#간단 꿀조합'],
    name: '진저라떼',
    date: '2020-11-03',
    view: 111,
    ctg: ['음료', '유제품', '방법기타', '스피드', '간단 꿀조합', '간식', '도구기타'],
    recipe: ['따뜻한 생강청과', '우유를 넣고', '섞어줍니다'],
  },
  {
    src: require('../img/카레 토마토솥밥.jpg'),
    id: 65,
    hash: ['#토마토', '#카레', '#솥밥', '#건강', '#비건', '#채식'],
    name: '카레 토마토솥밥',
    date: '2020-11-09',
    view: 131,
    ctg: ['밥', '채소', '방법기타', '채식', '다이어트', '냄비'],
    recipe: ['불린 쌀과', '손질한 토마토와 채소들을', '카레와 함께 솥에 끓여줍니다'],
  },
  {
    src: require('../img/쨈 아이스크림.jpg'),
    id: 66,
    hash: ['#오뚜기쨈', '#딸기쨈', '#포도쨈', '#디저트', '#우유', '#키즈푸드'],
    name: '쨈 아이스크림',
    date: '2021-01-01',
    view: 400,
    ctg: ['디저트', '유제품', '방법기타', '키즈푸드', '파티', '간식', '키즈셰프', '크리스마스', '생일', '도구기타'],
    recipe: ['노른자와 설탕을 넣고 섞습니다', '중탕하면서 우유를 조금씩 넣습니다', '잼을 넣고 냉동고에 얼려줍니다'],
  },
  {
    src: require('../img/옥수수 푸딩.jpg'),
    id: 67,
    hash: ['#옥수수푸딩', '#콘스프', '#단짠단짠', '#옥수수스프', '#파티', '#간식'],
    name: '옥수수 푸딩',
    date: '2021-01-02',
    view: 356,
    ctg: ['디저트', '유제품', '방법기타', '키즈푸드', '파티', '간식', '키즈셰프', '크리스마스', '생일', '도구기타'],
    recipe: ['우유, 바닐라빈을 끓이고 미지근하게 식힙니다', '식은 무스에 섞어 푸딩무스를 만듭니다', '검은깨와 남은 스위트콘을 올려 완성합니다'],
  },
  {
    src: require('../img/피자 토스트.jpg'),
    id: 68,
    hash: ['#피자소스', '#샌드위치', '#식빵요리', '#브런치', '#키즈셰프', '#키즈푸드'],
    name: '피자 토스트',
    date: '2021-01-03',
    view: 465,
    ctg: ['디저트', '재료기타', '방법기타', '키즈푸드', '파티', '간식', '키즈셰프', '크리스마스', '생일', '도구기타'],
    recipe: ['빵위에 토마토 소스를 펴발라줍니다', '치즈와 햄, 채소들을 토핑해줍니다', '오븐이나 전자레인지에 돌려줍니다'],
  },
  {
    src: require('../img/한라봉소르베.jpg'),
    id: 69,
    hash: ['#한라봉소르베', '#홈디저트', '#파티메뉴', '#마말레이드', '#제주담음', '#요린이'],
    name: '한라봉소르베',
    date: '2021-11-11',
    view: 266,
    ctg: ['디저트', '재료기타', '방법기타', '요린이를 위하여', '파티', '간식', '크리스마스', '생일', '도구기타'],
    recipe: ['제주 한라봉마말레이드와 물을 1:1 비율로 섞은 후 냉동실에 얼립니다', '얼린 주스를 믹서기에 곱게 갈아줍니다', '그릇에 옮겨 완성합니다'],
  },
  {
    src: require('../img/데리야끼치킨 뚱샌드위치.jpg'),
    id: 70,
    hash: ['#허니머스타드', '#올리고당', '#마요네스', '#골드마요네스', '#데리야끼소스', '#샌드위치'],
    name: '데리야끼치킨 뚱샌드위치',
    date: '2021-09-30',
    view: 465,
    ctg: ['종류기타', '닭고기', '방법기타', '파티', '간식', '도시락', '도구기타'],
    recipe: ['닭가슴살과 채 썬 양파에 데리야끼소스를 버무립니다', '팬에 마요네즈를 넣고 버무린 양념을 넣습니다', '볶은 양념을 빵과 야채와 조합해서 올려줍니다'],
  },
  {
    src: require('../img/순두부 열라면.jpg'),
    id: 71,
    hash: ['#순두부', '#열라면', '#역주행', '#순두부열라면', '#매운맛', '#화끈'],
    name: '순두부 열라면',
    date: '2021-10-23',
    view: 465,
    ctg: ['면', '재료기타', '국탕찌개', '야식', '캠핑', '간단 꿀조합', '부드러운 식감', '냄비'],
    recipe: ['끓는 물에 건더기, 스프, 순두부를 넣은뒤 조각내줍니다', '물이 끓으면 면을 넣고 기호에 따라서 다진 마늘을 넣어줍니다', '후추와 대파를 넣고 면이 어느 정도 익으면 계란 한 알을 넣습니다'],
  },
  {
    src: require('../img/쿠시아게 카레퐁듀.jpg'),
    id: 72,
    hash: ['#카레', '#카레퐁듀', '#오뚜기카레', '#캠핑', '#집들이', '#야식'],
    name: '쿠시아게 카레퐁듀',
    date: '2021-11-03',
    view: 465,
    ctg: ['종류기타', '재료기타', '방법기타', '캠핑', '야식', '카레데이', '후라이팬'],
    recipe: ['파프리카 1개를 잘게 잘라 볶은 후 믹서기에 갈아줍니다', '팬에 오일을 약간 두른 후 양파를 갈색빛이 날 때까지 볶습니다', '볶은 양파에 물 500ml를 넣고 카레 100g을 넣고 걸쭉하게 끓여줍니다'],
  },
  {
    src: require('../img/비빔만두.jpg'),
    id: 73,
    hash: ['#만두', '#야식'],
    name: '비빔만두',
    date: '2021-05-03',
    view: 465,
    ctg: ['종류기타', '채소', '구이', '야식', '간식', '에어프라이어', '후라이팬'],
    recipe: ['양배추, 오이, 양파, 깻잎은 얇게 채 썰어 준비합니다', '만두는 에어프라이어 180도에 7분간 조리하거나 팬에 구워줍니다', '볼에 준비한 채소와 파채양념, 참기름, 식초를 버무립니다'],
  },
];

export default allDataList;
